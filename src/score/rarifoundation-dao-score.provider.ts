...
export abstract class RariFoundationDaoScoreProvider
  ...
  constructor(private readonly resourceName = 'rarifoundation') {
    super(resourceName);
  }
  ...
  getForumScore(stat: Partial<DelegateStat>): number {
    ...
    return (
      Math.round(
        coalesce(stat.proposalsInitiated, 0) * coalesce(lifetime.proposalsInitiated, 1) +
        coalesce(stat.proposalsDiscussed, 0) * coalesce(lifetime.proposalsDiscussed, 1) +
        coalesce(stat.forumPostCount, 0) * coalesce(lifetime.forumPostCount, 1) +
        coalesce(stat.forumTopicCount, 0) * coalesce(lifetime.forumTopicCount, 1) +
        coalesce(stat.forumLikesReceived, 0) * coalesce(lifetime.forumLikesReceived, 1) +
        coalesce(stat.forumPostsReadCount, 0) * coalesce(lifetime.forumPostsReadCount, 1)
      ) || 0
    );
  }
